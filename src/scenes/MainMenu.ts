...
export class MainMenu extends Scene {
    ...
    for (let i = 0; i < this.lines.length; i++) {
      for (let j = i + 1; j < this.lines.length; j++) {
        ...
        if (
          intersection &&
          intersection.x > 50 &&
          intersection.x < this.WIDTH - 50 &&
          intersection.y > 50 &&
          intersection.y < this.HEIGHT - 50
        ) {
          const tooClose = this.nodes.some(
            (n) => Math.hypot(n.x - intersection.x, n.y - intersection.y) < 80
          );
  ...
  private createMenuOptions(): void {
    ...
    this.menuOptions.forEach((option) => {
      ...
      option.textObject = this.add
        .text(pos.x, pos.y - 35, option.text, {
  ...
  private drawLines(): void {
    ...
    this.lines.forEach((line) => {
      ...
      this.graphics.moveTo(
        Math.floor(line.start.x) + 0.5,
        Math.floor(line.start.y) + 0.5
      );
      this.graphics.lineTo(
        Math.floor(line.end.x) + 0.5,
        Math.floor(line.end.y) + 0.5
      );
