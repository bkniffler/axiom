...
export function layoutPlanetInfoPanel(
  params: LayoutPlanetInfoPanelParams
): PlanetInfoPanelLayout {
  ...
  const maxPanelWidth = Math.max(
    minPanelWidth,
    Math.min(340, params.viewportWidth - marginRight - 20)
  );
  ...
  panelWidth = Math.max(
    minPanelWidth,
    Math.min(maxPanelWidth, widestLine + paddingX * 2 + ps * 2)
  );
  ...
  if (panelHeight > maxPanelHeight) {
    const available = Math.max(
      1,
      Math.floor((maxPanelHeight - baseHeight) / lineHeight)
    );
