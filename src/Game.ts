...
import type { MenuCallbacks } from './SkiaMenu';
...
const startGame = async (parent: string) => {
  ...
  const requestStart = () => {
    ...
    startAudio();
    if (splashHint)
      splashHint.textContent = sceneReady ? 'Starting…' : 'Loading…';
    tryRevealSplash();
  };
  ...
  let rimStep = clampInt(
    Number(localStorage.getItem('axiom.menu.rimStep') ?? 4),
    1,
    TOTAL_RIM_STEPS
  );
  ...
  musicBtn.addEventListener('mouseleave', () => {
    ...
    musicBtn.style.color = strudelAudio.isMuted()
  ...
  const orbitalToggle = createToggle(
    'Orbital Lines',
    orbitalLinesEnabled,
    (checked) => {
  ...
  settingsBtn.addEventListener('click', (e) => {
    ...
    settingsBtn.style.borderColor = settingsOpen
      ? 'rgba(255,255,255,0.6)'
      : 'rgba(255,255,255,0.3)';
    settingsBtn.style.color = settingsOpen
  ...
  if (!startRequested) {
    if (splashHint)
      splashHint.textContent = 'Click, tap, or press Enter to start';
  }
  tryRevealSplash();
  void audioInit;
};
