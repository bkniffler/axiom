...
import type {
  Rgb,
  TrafficPlanetSnapshot,
  TransferTrafficColors,
  TransferTrafficDrawParams,
  TransferTrafficUpdateParams,
  Vec2,
} from './types';
...
export class TransferTrafficSystem {
  ...
  reset(initialDelaySeconds = 0): void {
    this.ships = [];
  ...
  update(params: TransferTrafficUpdateParams): void {
    ...
    if (
      ...
      elapsedSeconds >= this.nextSpawnAt
    ) {
    ...
    for (let i = this.ships.length - 1; i >= 0; i--) {
      ...
      if (s.state === 'path') {
        ...
        const ahead = samplePath(
          s.path,
          s.cumulative,
          Math.min(s.totalLength, s.progress + this.pixelSize * 10)
        );
        ...
        if (dist <= dockRadius) {
          ...
          s.pos = {
            x: s.pos.x + dir.x * s.speed * deltaSeconds,
            y: s.pos.y + dir.y * s.speed * deltaSeconds,
          };
          s.heading = dir;
        }
      } else {
        ...
        if (
          s.arrivedTime !== null &&
          elapsedSeconds - s.arrivedTime > this.dockTimeSeconds
        ) {
          this.ships.splice(i, 1);
        }
      }
  ...
  private spawnShip(
    ...
    const startPt = {
      x: start.x * (1 + startOffset / startR),
      y: start.y * (1 + startOffset / startR),
    };
    const goalPt = {
      x: goal.x * (1 + goalOffset / goalR),
      y: goal.y * (1 + goalOffset / goalR),
    };
  ...
  private getPlanetApproachPoint(
    planets: TrafficPlanetSnapshot[],
    index: number
  ): Vec2 | null {
