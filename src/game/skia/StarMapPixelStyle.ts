...
import { advanceAsteroidBelts, advanceOrbitingObjects, advancePlanetOrbits } from './starmap/sim/orbits';
import { advanceComets } from './starmap/sim/deepSpace';
import { smoothFactor } from './starmap/math/smoothing';
import { createCamera, resetCamera, setCameraTarget, stepCamera } from './starmap/camera/controller';
...
import { drawPlanets, drawPlanetHoverLabel } from './starmap/render/passes/planets';
...
import { drawParticles, type Particle } from './starmap/render/passes/particles';
...
export class StarMapPixelStyle extends SkiaMenuStyle {
    ...
    private setupKeyHandlers(): void {
        this.keyHandler = (e: KeyboardEvent) => {
            if (e.key === 'r' || e.key === 'R' || e.key === '0' || e.key === 'Escape') {
                this.resetCamera(true);
            }
        };
    ...
    public setUnlockStep(step: number, totalSteps: number = 10): void {
        ...
        const eased = Math.pow(Math.max(0, Math.min(1, t)), 1.25);

        const minR = this.unlockMinRadius > 0 ? this.unlockMinRadius : Math.max(1, this.systemBaseOrbit * 1.1);
        const maxR = this.unlockMaxRadius > 0 ? this.unlockMaxRadius : Math.max(minR + 1, this.systemBaseOrbit * 5);
    ...
    private worldToScreen(wx: number, wy: number, z: number, parallax: number = 1): { x: number; y: number } {
    ...
    private pixelTextWidth(text: string, pixelSize: number = this.PIXEL_SIZE): number {
        return measurePixelTextWidth(text, pixelSize);
    }

    private drawPixelAtSize(x: number, y: number, pixelSize: number, color: number[], opacity: number): void {
        this.renderer?.drawPixelAtSize(x, y, pixelSize, color, opacity);
    }

    private drawPixelTextScaled(x: number, y: number, text: string, color: number[], opacity: number, pixelSize: number): void {
        if (!this.renderer) return;
        drawPixelText(this.renderer, x, y, text, color, opacity, pixelSize);
    }

    protected override drawTitle(opacity: number = 1): void {
        ...
        this.drawPixelTextScaled(x, y, title, this.COLORS.white, 0.35 * opacity, pixelSize);
    }

    protected override drawMenu(opacity: number = 1): void {
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            this.drawPixelTextScaled(x, y, text, color, baseOpacity * opacity, pixelSize);

            if (selected) {
                const indicatorX = x - pixelSize * 3.5;
                const indicatorY = y + pixelSize * 2;
                this.drawPixelAtSize(indicatorX, indicatorY, pixelSize, this.COLORS.white, 0.85 * opacity);
                this.drawPixelAtSize(indicatorX - pixelSize, indicatorY, pixelSize, this.COLORS.lightGray, 0.45 * opacity);
            }
        }
    }
    ...
    private updateHoveredPlanet(): void {
        ...
        // Update cursor
        document.body.style.cursor = this.hoveredPlanetIndex >= 0 || this.hoveredSun ? 'pointer' : 'default';
    }

    private drawPixel(x: number, y: number, color: number[], opacity: number = 1): void {
        this.renderer?.drawPixel(x, y, color, opacity);
    }

    private drawPixelCircle(cx: number, cy: number, radius: number, color: number[], opacity: number = 1, filled: boolean = true): void {
    ...
    private initPlanets(): void {
        ...
        const planetData: Array<{
            ...
            { name: 'Meridia', type: 'Ice World', color: this.COLORS.cyan, description: 'A frozen world with vast ice plains and hidden oceans beneath the surface.', orbitFactor: 0.55, sizePx: 3.2 },
            { name: 'Solara', type: 'Desert Planet', color: this.COLORS.orange, description: 'Scorched by twin suns, rich in rare minerals and ancient ruins.', orbitFactor: 0.75, sizePx: 3.8, hasMoon: true },
            { name: 'Verdant', type: 'Garden World', color: this.COLORS.blue, description: 'A temperate world wrapped in dense cloud bands and emerald continents.', orbitFactor: 0.95, sizePx: 4.2 },
            { name: 'Pyronis', type: 'Volcanic', color: this.COLORS.red, description: 'An unstable world of constant eruptions and rivers of molten rock.', orbitFactor: 1.12, sizePx: 4.6 },
            { name: 'Nebulos', type: 'Gas Giant', color: this.COLORS.purple, description: 'A massive gas giant with swirling storms and a spectacular ring system.', orbitFactor: 1.35, sizePx: 6.2, hasRing: true },
            { name: 'Aquaris', type: 'Ocean World', color: this.COLORS.blue, description: 'Covered entirely by water, home to bioluminescent life forms.', orbitFactor: 1.6, sizePx: 4.8 },
            { name: 'Kryos', type: 'Cryo Dwarf', color: this.COLORS.lightGray, description: 'A tiny icy dwarf with a glittering dust halo and silent canyons.', orbitFactor: 1.9, sizePx: 2.6 },
            { name: 'Aurelion', type: 'Storm Giant', color: this.COLORS.orange, description: 'Bands of amber storms race across a crushing atmosphere.', orbitFactor: 2.25, sizePx: 6.8, hasRing: true },
            { name: 'Umbra', type: 'Shadow Planet', color: this.COLORS.midGray, description: 'A dim world that eats starlight; its nightside never ends.', orbitFactor: 2.6, sizePx: 4.3 },
            { name: 'Cindara', type: 'Ash World', color: this.COLORS.darkGray, description: 'A carbon-black sphere with thin rings of powdered stone.', orbitFactor: 2.95, sizePx: 3.9, hasRing: true },
            { name: 'Heliox', type: 'Helium Giant', color: this.COLORS.cyan, description: 'A pale giant with high-altitude lightning and shimmering auroras.', orbitFactor: 3.3, sizePx: 7.4 },
            { name: 'Driftveil', type: 'Frost Ring', color: this.COLORS.lightGray, description: 'A distant world surrounded by needle-thin ice rings.', orbitFactor: 3.7, sizePx: 4.1, hasRing: true },
        ];
    ...
    private initUnlockDefaults(): void {
        const maxPlanet = this.planets.reduce((m, p) => Math.max(m, p.orbitRadius + p.size), 0);
        const maxBelt = this.asteroidBelts.reduce((m, b) => Math.max(m, b.orbitRadius + b.thickness), 0);
        const maxOrbiting = this.orbitingObjects.reduce((m, o) => Math.max(m, o.orbitRadius + o.size), 0);
        ...
        this.unlockMaxRadius = Math.max(this.unlockMinRadius + 1, Math.max(maxPlanet, maxBelt, maxOrbiting, maxDeep) * 1.05);
    ...
    private initAsteroidBelts(): void {
        ...
        const belts: Array<{
            ...
            { orbitFactor: 1.48, thicknessPx: 22, count: 90, speed: 0.07, color: this.COLORS.darkGray, seed: 321 },
            { orbitFactor: 2.85, thicknessPx: 28, count: 120, speed: 0.04, color: this.COLORS.midGray, seed: 987 },
        ];

        for (const belt of belts) {
            const rng = this.lcg(belt.seed);
            const orbitRadius = baseOrbit * belt.orbitFactor;
            const asteroids: { angleOffset: number; radiusOffset: number; size: number }[] = [];
    ...
    private initOrbitingObjects(): void {
        ...
        const objects: Array<{
            ...
            { kind: 'station', orbitFactor: 1.25, orbitSpeed: 0.075, size: this.PIXEL_SIZE * 2.2, color: this.COLORS.lightGray },
            { kind: 'probe', orbitFactor: 1.78, orbitSpeed: 0.09, size: this.PIXEL_SIZE * 1.2, color: this.COLORS.cyan, blinkSpeed: 4.5 },
            { kind: 'ship', orbitFactor: 2.15, orbitSpeed: 0.06, size: this.PIXEL_SIZE * 2.0, color: this.COLORS.white },
            { kind: 'probe', orbitFactor: 3.25, orbitSpeed: 0.03, size: this.PIXEL_SIZE * 1.1, color: this.COLORS.orange, blinkSpeed: 3.2 },
        ];
    ...
    private initDeepSpaceObjects(): void {
        ...
        const makeNebula = (x: number, y: number, size: number, color: number[], opacity: number, seed: number): DeepSpaceObject => {
            const r = this.lcg(seed);
            const points: { dx: number; dy: number; opacity: number }[] = [];
            const count = 120;
            for (let i = 0; i < count; i++) {
                const a = r() * Math.PI * 2;
                const d = Math.pow(r(), 1.8) * size;
                points.push({
        ...
        this.deepSpaceObjects.push(
            makeNebula(-world * 1.55, -world * 0.85, 140, this.COLORS.purple, 0.22, 1001),
            makeNebula(world * 1.35, world * 1.15, 160, this.COLORS.blue, 0.2, 2002),
            { kind: 'roguePlanet', x: world * 1.5, y: -world * 1.05, size: this.PIXEL_SIZE * 7.5, color: this.COLORS.darkGray, opacity: 0.9 },
            { kind: 'derelict', x: -world * 1.25, y: world * 1.25, size: this.PIXEL_SIZE * 7, color: this.COLORS.midGray, opacity: 0.55 },
            { kind: 'beacon', x: world * 1.85, y: world * 0.2, size: this.PIXEL_SIZE * 2, color: this.COLORS.cyan, opacity: 0.75, blinkSpeed: 3.8 },
            { kind: 'beacon', x: -world * 1.85, y: -world * 0.15, size: this.PIXEL_SIZE * 2, color: this.COLORS.orange, opacity: 0.7, blinkSpeed: 2.9 },
            {
                kind: 'comet',
                x: -world * 2.1,
                y: world * (0.7 + rng() * 0.2),
    ...
    private drawNebulaShader(elapsed: number, transitionProgress: number): void {
        ...
        const fade = transitionProgress > 0 ? Math.max(0, 1 - transitionProgress * 1.2) : 1;
    ...
    protected animate(time: number): void {
        ...
        const deltaTimeRaw = this.lastTime > 0 ? (time - this.lastTime) / 1000 : 1 / 60;
        ...
        if (!this.particlesSpawned) {
            ...
            if (transitionProgress === 0) {
                advanceAsteroidBelts(this.asteroidBelts, deltaTime, this.ORBIT_SPEED_SCALE);
                advanceOrbitingObjects(this.orbitingObjects, deltaTime, this.ORBIT_SPEED_SCALE);
            }
        }
        ...
        if (!this.particlesSpawned && transitionProgress === 0 && this.followPlanetIndex !== null) {
        ...
        if (this.targetUnlockRadius > 0) {
            ...
            this.unlockRadius += (this.targetUnlockRadius - this.unlockRadius) * unlockFactor;
        }
        ...
        this.transferTraffic?.update({
            ...
            planets: this.planets.map((planet) => ({ pos: this.getPlanetWorldPosition(planet), size: planet.size })),
        });
            ...
            if (this.hoveredPlanetIndex >= 0 && this.selectedPlanetIndex < 0) {
                ...
                if (planet) {
                    drawPlanetHoverLabel({
                        ...
                        zoom: this.zoom,
                        pixelSize: this.PIXEL_SIZE,
                        worldToScreen: (wx: number, wy: number) => this.worldToScreen(wx, wy, this.zoom, 1),
                        color: this.COLORS.white,
                        opacity: 0.9,
                    });
                }
    ...
    private drawAllElements(elapsed: number, transitionProgress: number): void {
        ...
        if (this.renderer) {
            drawStars({
                ...
                worldToScreen: (wx: number, wy: number, parallax: number) => this.worldToScreen(wx, wy, this.zoom, parallax),
            });
        }
        ...
        if (this.particlesSpawned) {
            ...
            this.drawPixelCircle(cx, cy, singularitySize * this.PIXEL_SIZE, this.COLORS.darkGray, singularityOpacity * 0.4);
            this.drawPixelCircle(cx, cy, this.PIXEL_SIZE * 1.5 * z, this.COLORS.white, singularityOpacity);
        ...
        if (this.renderer) {
            drawOrbitPaths({
                ...
                zoom: z,
                pixelSize: this.PIXEL_SIZE,
                planets: this.planets.map((pl) => ({ orbitRadius: pl.orbitRadius, color: pl.color })),
        ...
        if (this.renderer) {
            drawAsteroidBelts({
                ...
                zoom: z,
                pixelSize: this.PIXEL_SIZE,
                worldToScreen: (wx: number, wy: number) => this.worldToScreen(wx, wy, z, 1),
            });
        }
        ...
        this.transferTraffic?.draw({
            ...
            pixelSize: this.PIXEL_SIZE,
            worldToScreen: (wx: number, wy: number) => this.worldToScreen(wx, wy, z, 1),
            drawPixel: (x: number, y: number, color: number[], opacity: number) => this.drawPixel(x, y, color, opacity),
        });
