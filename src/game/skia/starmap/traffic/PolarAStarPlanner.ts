...
export function planPolarAStarPath(params: PolarAStarPlanParams): Vec2[] {
    ...
    const maxOrbit = Math.max(startR, goalR, ...obstaclePoints.map((p) => worldRadius(p)));
    ...
    const toNode = (p: Vec2): number => {
        ...
        const ai = ((Math.round(a / angleStep) % angleCount) + angleCount) % angleCount;
    ...
    const heuristic = (a: Vec2, b: Vec2): number => Math.hypot(a.x - b.x, a.y - b.y);
    ...
    const neighborIds = (id: number): number[] => {
        ...
        if (ri > 0) out.push((ri - 1) * angleCount + a0, (ri - 1) * angleCount + a1);
        if (ri + 1 < ringCount) out.push((ri + 1) * angleCount + a0, (ri + 1) * angleCount + a1);
        return out;
    };
