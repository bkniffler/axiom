...
export function drawPlanetInfoPanel(params: DrawInfoPanelParams): void {
    ...
    params.renderer.fillRectLTRB(layout.panelX, layout.panelY, layout.panelX + layout.panelWidth, layout.panelY + layout.panelHeight, [0, 0, 0], 0.85);

    const ps = params.pixelSize;
    const borderColor = params.selected.color;
    const borderOpacity = 0.6;

    params.renderer.fillRectLTRB(layout.panelX, layout.panelY, layout.panelX + layout.panelWidth, layout.panelY + ps, borderColor, borderOpacity);
    ...
    params.renderer.fillRectLTRB(layout.panelX, layout.panelY, layout.panelX + ps, layout.panelY + layout.panelHeight, borderColor, borderOpacity);
    params.renderer.fillRectLTRB(
    ...
    drawPixelText(params.renderer, layout.textX, layout.nameY, layout.name, params.selected.color, 1, ps);
    drawPixelText(params.renderer, layout.textX, layout.typeY, layout.type, params.lightGray, 0.7, ps);

    let y = layout.descY;
    for (const line of layout.descLines) {
        drawPixelText(params.renderer, layout.textX, y, line, params.midGray, 0.6, ps);
        y += layout.lineHeight;
    }
}
