...
export function getPlanetWorldPosition(planet: { orbitRadius: number; angle: number }): { x: number; y: number } {
    return { x: Math.cos(planet.angle) * planet.orbitRadius, y: Math.sin(planet.angle) * planet.orbitRadius };
}
...
export function drawPlanets(params: DrawPlanetsParams): void {
    ...
    for (let i = 0; i < params.planets.length; i++) {
        ...
        if (params.shakePhase > 0 && shakeOffset) {
            const planetShakePhase = Math.max(0, params.shakePhase - shakeOffset.delay) / (1 - shakeOffset.delay);
        ...
        params.renderer.drawPixel(px - scaledSize * 0.5, py - scaledSize * 0.5, params.white, 0.5);

        if (planet.hasRing) {
            const ringWidth = scaledSize * 2.5;
            params.renderer.drawPixelLine(px - ringWidth, py, px + ringWidth, params.lightGray, 0.6);
        }
        ...
        if (isSelected) {
            ...
            params.renderer.drawPixelCircle(px, py, ringRadius, ringColor, ringOpacity, false);
...
export function drawPlanetHoverLabel(params: DrawHoverLabelParams): void {
    ...
    const labelY = py - 8;
    drawPixelText(params.renderer, labelX, labelY, params.planet.name.toUpperCase(), params.color, params.opacity, params.pixelSize);
}
