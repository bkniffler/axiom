...
export function drawOrbitingObjects(params: DrawOrbitingObjectsParams): void {
    ...
    for (const obj of params.objects) {
        ...
        x += Math.sin(params.elapsedSeconds * 1.7 + obj.orbitRadius * 0.01) * params.pixelSize * 0.15;
        y += Math.cos(params.elapsedSeconds * 1.9 + obj.orbitRadius * 0.01) * params.pixelSize * 0.15;
        ...
        switch (obj.kind) {
            ...
            case 'probe': {
                const blink = obj.blinkSpeed ? Math.sin(params.elapsedSeconds * obj.blinkSpeed) : 1;
