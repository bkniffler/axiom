...
export function drawStars(params: DrawStarsParams): void {
    ...
    for (const star of params.stars) {
        const twinkle = Math.sin(params.elapsedSeconds * star.twinkleSpeed + star.twinklePhase);
        if (twinkle < -0.3) continue;

        const { x: sx, y: sy } = params.worldToScreen(star.x - params.cx, star.y - params.cy, params.starParallax);
        let x = sx;
        let y = sy;
