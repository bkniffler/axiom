...
export type DeepSpaceObject =
    ...
    | { kind: 'roguePlanet'; x: number; y: number; size: number; color: Rgb; opacity: number }
    | { kind: 'derelict'; x: number; y: number; size: number; color: Rgb; opacity: number }
    | { kind: 'beacon'; x: number; y: number; size: number; color: Rgb; opacity: number; blinkSpeed: number }
    | { kind: 'comet'; x: number; y: number; size: number; color: Rgb; opacity: number; vx: number; vy: number; tail: number };
...
export interface DrawDeepSpaceParams {
    ...
    cy: number;
    maxDim: number;
    worldToScreen: (wx: number, wy: number, parallax: number) => { x: number; y: number };
    lightGray: Rgb;
    white: Rgb;
}

function collapsePoint(x: number, y: number, cx: number, cy: number, maxDim: number, z: number, pull: number, collapse: number): { x: number; y: number } {
    const dx = x - cx;
    const dy = y - cy;
...
export function drawDeepSpaceObjects(params: DrawDeepSpaceParams): void {
    for (const obj of params.objects) {
        ...
        if (params.transitionProgress > 0) {
            const collapsed = collapsePoint(x, y, params.cx, params.cy, params.maxDim, params.zoom, params.pull, params.collapse);
            x = collapsed.x;
            y = collapsed.y;
            opacity *= 0.55;
        }

        const objRadius = (obj.kind === 'nebula' ? obj.size : obj.size * 2) * params.zoom;
        const margin = objRadius + 60;
        if (x < -margin || x > params.cx * 2 + margin || y < -margin || y > params.cy * 2 + margin) continue;
        ...
        switch (obj.kind) {
            case 'nebula': {
                ...
                for (let i = 0; i < obj.points.length; i++) {
                    ...
                    if (o < 0.05) continue;
                    if (((i + tick) % 6) !== 0) params.renderer.drawPixel(px, py, obj.color, o);
                }
                break;
            }
            case 'roguePlanet': {
                params.renderer.drawPixelCircle(x, y, obj.size * params.zoom, obj.color, opacity, true);
                params.renderer.drawPixel(x - obj.size * 0.4 * params.zoom, y - obj.size * 0.4 * params.zoom, params.lightGray, opacity * 0.25);
                break;
            }
            case 'derelict': {
                const ps = params.pixelSize * params.zoom;
                const s = Math.max(ps, obj.size * params.zoom);
                params.renderer.drawPixelLine(x - s, y, x + s, obj.color, opacity * 0.6);
                params.renderer.drawPixelLine(x - s * 0.5, y - ps, x + s * 0.5, obj.color, opacity * 0.6);
                params.renderer.drawPixel(x + s * 0.2, y + ps, params.lightGray, opacity * 0.35);
                params.renderer.drawPixel(x - s * 0.6, y + ps * 2, obj.color, opacity * 0.3);
                break;
            }
            case 'beacon': {
                ...
                const ps = params.pixelSize * params.zoom;
                params.renderer.drawPixelCircle(x, y, obj.size * params.zoom, obj.color, o, true);
