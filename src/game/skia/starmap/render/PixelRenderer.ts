...
export interface PixelRenderer {
    ...
    drawPixelCircle(cx: number, cy: number, radius: number, color: Rgb, opacity?: number, filled?: boolean): void;
    drawPixelLine(x1: number, y: number, x2: number, color: Rgb, opacity?: number): void;
    drawPixelVertLine(x: number, y1: number, y2: number, color: Rgb, opacity?: number): void;
