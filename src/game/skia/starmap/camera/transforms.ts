...
export function worldToScreen(wx: number, wy: number, params: CameraTransformParams): { x: number; y: number } {
...
export function screenToWorld(sx: number, sy: number, params: CameraTransformParams): { x: number; y: number } {
    const cx = params.viewportWidth / 2;
    const cy = params.viewportHeight / 2;
    const parallax = params.parallax ?? 1;
    return {
