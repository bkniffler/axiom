...
export interface SpawnParticlesParams {
    planets: SpawnPlanet[];
    pixelSize: number;
...
export function spawnPlanetExplosionParticles(params: SpawnParticlesParams): Particle[] {
    const rand = params.rand ?? Math.random;
    const particles: Particle[] = [];
    const particlesPerPlanet = Math.max(10, Math.floor(params.targetParticleCount / Math.max(1, params.planets.length)));
