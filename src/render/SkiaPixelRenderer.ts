...
export class SkiaPixelRenderer implements PixelRenderer {
  ...
  drawPixel(x: number, y: number, color: Rgb, opacity = 1): void {
    const sx = this.snap(x);
    const sy = this.snap(y);
    this.paint.setColor(this.CK.Color(color[0], color[1], color[2], opacity));
    this.canvas.drawRect(
      this.CK.LTRBRect(sx, sy, sx + this.pixelSize, sy + this.pixelSize),
      this.paint
    );
  }
  ...
  drawStarPixel(x: number, y: number, opacity: number): void {
    ...
    this.paint.setColor(
      this.CK.Color(this.white[0], this.white[1], this.white[2], opacity)
    );
    this.canvas.drawRect(
      this.CK.LTRBRect(sx, sy, sx + ps, sy + ps),
      this.paint
    );
  }
  ...
  drawPixelAtSize(
    ...
    this.paint.setColor(this.CK.Color(color[0], color[1], color[2], opacity));
    this.canvas.drawRect(
      this.CK.LTRBRect(sx, sy, sx + pixelSize, sy + pixelSize),
      this.paint
    );
  }
  ...
  drawPixelCircle(
    ...
    const drawPixelAt = (x: number, y: number) => {
      this.paint.setColor(this.CK.Color(color[0], color[1], color[2], opacity));
      this.canvas.drawRect(
        this.CK.LTRBRect(x, y, x + this.pixelSize, y + this.pixelSize),
        this.paint
      );
    };
  ...
  drawPixelLine(
    ...
    for (let x = sx1; x <= sx2; x += this.pixelSize) {
      this.canvas.drawRect(
        this.CK.LTRBRect(x, sy, x + this.pixelSize, sy + this.pixelSize),
        this.paint
      );
    }
  }
  ...
  drawPixelVertLine(
    ...
    for (let y = sy1; y <= sy2; y += this.pixelSize) {
      this.canvas.drawRect(
        this.CK.LTRBRect(sx, y, sx + this.pixelSize, y + this.pixelSize),
        this.paint
      );
    }
  }
